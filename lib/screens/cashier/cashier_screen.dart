...
class _CashierScreenState extends State<CashierScreen> {
  ...
  Future<void> _initBluetooth() async {
    ...
    bluetooth.onStateChanged().listen((state) {
      if (mounted) {
        setState(() {
          _Connected =
              state == BlueThermalPrinter.CONNECTED; // cek status koneksi
        });
      }
    });
  }
  ...
  void _removeFromCart(Product product) {
    setState(() {
      if (_cart.containsKey(product) && _cart[product]! > 1) {
        _cart[product] =
            _cart[product]! -
            1; // perbedaan bankopearator(!) itu wajib ada valuenya, kalau not kebalikannya
      } else {
        _cart.remove(
          product,
        ); // ini untuk dijalankan kalau misalkan codingannya ada error
      }
    });
  }
  ...
  void _handlePrint() async {
    ...
    if (_selectedDevice != null && await bluetooth.isConnected == true) {
      ...
      bluetooth.printCustom("IDN CAFE", 3, 1); // Judul besar
      bluetooth.printNewLine();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ProductCard(
                  ...
                  onRemoved: () => _removeFromCart(product),
                );
              },
            ),
          ),
          ...
          // Bottom Sheet panel
          CheckoutPanel(total: _calculateTotal(), onPressed: _handlePrint),
        ],
      ),
    );
  }
